...
@endsection
@section('content')
<h2 class="ui header dividing">Upload Video</h2>
<form class="ui form" method="POST" id="upload" action="{{ url('user/upload') }}">
 {!! csrf_field() !!}
  ...
  <div class="field {{ $errors->has('link') ? 'error' : '' }}">
    <div class="ui labeled input large">
      ...
      <input type="text" name="link" placeholder="Youtube Url: e.g. https://www.youtube.com/watch?v=w14zUTXOhYE" value="{{ old('link') }}">
    </div>
  </div>
  ...
  <button class="ui button primary large" type="submit">Upload</button>
</form>
@endsection
